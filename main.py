...
def get_dataset_dir(args):
    ...
    dataset_dir = (
        args.dataset_dir if args.dataset_dir else os.path.join(home, "datasets", "lfw")
    )
...
def get_log_dir(args):
    log_dir = (
        args.log_dir
        if args.log_dir
        else os.path.join(os.path.dirname(os.path.realpath(__file__)), "logs")
    )
...
def train(args):
    ...
    training_dataloader = torch.utils.data.DataLoader(
        training_dataset, batch_size=args.batch_size, num_workers=6, shuffle=True
    )

    validation_dataloader = torch.utils.data.DataLoader(
        validation_dataset, batch_size=args.batch_size, num_workers=6, shuffle=False
    )
...
def evaluate(args):
    ...
    pairs_path = args.pairs if args.pairs else os.path.join(dataset_dir, "pairs.txt")
    ...
    tpr, fpr, accuracy, best_thresholds = compute_roc(distances, matches, thresholds)
...
def verify(args):
    ...
    image_a = transform_for_infer(model_class.IMAGE_SHAPE)(image_loader(image_a))
    image_a = image_a.unsqueeze(0)
    image_a = image_a.to(device)
    # print(f"Image A shape : {image_a.shape}")
    # image_a = torch.randn(3, 96, 128)
    image_b = transform_for_infer(model_class.IMAGE_SHAPE)(image_loader(image_b))
    ...
    # print(f"Enbedding A : {embedings_a.shape} | Embedding B : {embedings_b.shape}")
    # print(f"A - B = {torch.sum(torch.pow(embedings_a - embedings_b, 2))}")
    # print(out)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume", type=str, help="model path to the resume training", default=False
    )
    parser.add_argument(
        "--dataset_dir",
        type=str,
        help="directory with lfw dataset" " (default: $HOME/datasets/lfw)",
    )
    parser.add_argument(
        "--weights",
        type=str,
        help="pretrained weights to load " "default: ($LOG_DIR/resnet18.pth)",
    )
    parser.add_argument(
        "--evaluate", type=str, help="evaluate specified model on lfw dataset"
    )
    parser.add_argument(
        "--pairs",
        type=str,
        help="path of pairs.txt " "(default: $DATASET_DIR/pairs.txt)",
    )
    parser.add_argument(
        "--roc",
        type=str,
        help="path of roc.png to generated " "(default: $DATASET_DIR/roc.png)",
    )
    parser.add_argument(
        "--verify-model",
        type=str,

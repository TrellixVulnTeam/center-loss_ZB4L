...
class Trainer(object):
    ...
    def run_epoch(self, mode):
        ...
        with torch.set_grad_enabled(mode == "train"):
            for images, targets, names in dataloader:
                ...
                print(
                    "[{}:{}] cross entropy loss: {:.8f} - center loss: "
            ...
            print(
                ...
                "top1 acc: {:.4f} % - top3 acc: cencelled".format(
                    mode,
                    self.current_epoch,
                    cross_entropy_loss.item(),
                    center_loss.item(),
    ...
    def _get_matches(self, targets, logits, n=1):
        ...
        matches = torch.sum(preds == targets_repeated,
                            dim=1).nonzero().size()[0]
        return matches
    ...
    def persist(self, is_best=False):
        ...
        if not os.path.isdir(model_dir):
            os.mkdir(model_dir)
        file_name = ("epoch_{}_best.pth.tar" if is_best else "epoch_{}.pth.tar").format(
            self.current_epoch
        )
        ...
        # torch.save(
        #     state, '/home/elmo/Desktop/arc-face-projects/center-loss/logs/models/last.pth')

        # print(self.model.to(torch.device("cpu")))
        model_state_dict = self.model
        model_state_dict.to(torch.device("cpu"))
        print(f"\n--- Save model : {state_path} ---")
        torch.save(model_state_dict.state_dict(), state_path)
        # print("\n--- Done saving...")
        # self.model.to(torch.device("gpu"))
        print("End training..")
